...
/* POSIX.2 CRC32 checksum algorithm from GNU coreutils (GPLV3), suitably
   massaged to compute a CRC from data in memory. */
...
/* Written by Q. Frank Xia, qx@math.columbia.edu.  Cosmetic changes and
   reorganization by David MacKenzie, djm@gnu.ai.mit.edu. */
...
#define INT_STRLEN_BOUND(t) ((sizeof (t) * 8 - 1) * 302 / 1000 + 2)
#define INT_BUFSIZE_BOUND(t) (INT_STRLEN_BOUND (t) + 1)

static char *
uint64tostr (uint64_t i, char *buf)
{
  char *p = buf + INT_STRLEN_BOUND (uint64_t);
  *p = 0;
  if (i < 0) {
    do
      *--p = '0' - i % 10;
...
uint32_t
cksum (size_t n, unsigned char *buf)
{
  unsigned char *cp = buf;
  uint32_t crc = 0;
  uint64_t length = n;
  char length_buf[INT_BUFSIZE_BOUND (uint64_t)];

  while (n--)
    crc = (crc << 8) ^ crctab[((crc >> 24) ^ *cp++) & 0xFF];

  uint64tostr (length, length_buf);
