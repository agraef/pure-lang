...
/* Internal Faust interface (Pure 0.44 and later). These provide the necessary
   helper functions to access the control variables and other useful
   information about a Faust dsp. NOTE: You should never have to call any of
   these directly; instead, applications should use the corresponding
   dspname::info and dspname::ui wrapper functions generated by the Pure
   compiler when the Faust dsp is loaded. */

void *faust_float_ui();
void *faust_double_ui();
void faust_free_ui(void *ui);
pure_expr *faust_get_info(int n_in, int n_out, int flags);
pure_expr *faust_get_ui(void *ui);
