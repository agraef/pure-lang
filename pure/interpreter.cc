...
void interpreter::compiler(const char *out)
{
  ...
  time_t t; time(&t);
  code << "; ModuleID = '" << module->getModuleIdentifier() << "'\n";
  code << "; Generated by pure " << PACKAGE_VERSION << " " << ctime(&t);
  code << "target datalayout = \""
       << JIT->getTargetData()->getStringRepresentation() << "\"\n";
  code << "target triple = \"" << HOST << "\"\n\n";
  TypeSymbolTable &types = module->getTypeSymbolTable();
  code << "%struct.expr = type { i32, i32, %struct.expr *, %struct.expr * }\n";
  for (TypeSymbolTable::iterator it = types.begin(), end = types.end();
       it != end; ++it) {
